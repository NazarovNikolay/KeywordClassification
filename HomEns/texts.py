...
def method_keywords_to_text(
    ...
    В ноутбуках ``hom_ens`` это ``row_to_kw_text``: все фразы столбца,
    опционально с тегом источника, через ``"; "``. Подпространства нет —
    разнообразие даёт сам экстрактор, а не сэмплер.
    """
